...
def evaluate(df: pd.DataFrame, backend: CompletionBackend, show_commits: bool = False, suffix: str = "") -> Dict[str, float | int]:
    ...
    iterator: List[Tuple[str, str, str]] = list(zip(df["project_url"], df["commit_id"], df["func"]))

    for proj, cid, full_func in tqdm(iterator, desc="Evaluating", total=len(df)):
        ...
        predicted = backend.complete(proj, cid, prefix, suffix=suffix)
