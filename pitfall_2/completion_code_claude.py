#!/usr/bin/env python3
"""
PrimeVul *code-completion* evaluation — extended metrics version for Claude Haiku 3.5.
Adds:
...
Dependencies
------------
python >= 3.9; pandas, numpy, sklearn, tqdm, anthropic >= 0.25.0
...
MODEL, MAX_TOKENS = "claude-3-5-haiku-20241022", 4096
...
def evaluate(df: pd.DataFrame, name: str) -> dict:
    ...
    for func, proj, cid in tqdm(iterator, total=len(df), desc=f"{name:>18}"):
        ...
        try:
            ...
            print(f"Claude error – treating as empty: {exc}")
            pred = ""
