#!/usr/bin/env python3
"""
PrimeVul *commit message* completion evaluation for Claude Haiku 3.5.
Evaluates:
...
Dependencies:
 pandas, numpy, sklearn, tqdm, anthropic, python-Levenshtein (optional), nltk (optional)
"""
...
MODEL, MAX_TOKENS = "claude-3-5-haiku-20241022", 1024
...
def evaluate(df: pd.DataFrame, name: str) -> dict:
    ...
    for proj, cid, true_msg in tqdm(iterator, total=len(df), desc=f"{name:>18}"):
        ...
        try:
            ...
            print(f"Claude error – treating as empty: {e}")
            pred = ""
