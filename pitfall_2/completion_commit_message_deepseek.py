#!/usr/bin/env python3
"""primevul_deepseek_eval.py
PrimeVul *commit message* completion evaluation adapted for DeepSeek API.

Key changes versus the Anthropic version
---------------------------------------
* Uses the **OpenAI** Python SDK (which DeepSeek is compatible with) instead of `anthropic`.
* Reads `DEEPSEEK_API_KEY` (or falls back to `OPENAI_API_KEY`) from the environment.
* Configures the client with `base_url="https://api.deepseek.com"`.
* Rewrites the `complete()` helper to call `client.chat.completions.create`.
* Updates error handling to catch `openai.OpenAIError`.
* All other logic, metrics and dependencies are preserved.

Dependencies
------------
```bash
pip install openai pandas numpy scikit-learn tqdm python-Levenshtein nltk
```
"""
...
# Try to import the OpenAI-compatible SDK (works for DeepSeek)
try:
    from openai import OpenAI
    from openai import OpenAIError  # type: ignore
except ImportError as exc:  # pragma: no cover
    raise RuntimeError(
        "openai-python is required. Install with `pip install openai`."
    ) from exc

# ───── Debug flag ─────
PRINT_COMMIT_MESSAGES = True

# ───── Optional third-party metric libraries ─────
try:
    import Levenshtein

    lev_distance = Levenshtein.distance  # type: ignore[attr-defined]
except ImportError:
...
if API_KEY is None:
    raise RuntimeError("DEEPSEEK_API_KEY (or OPENAI_API_KEY) environment variable not set.")
...
def load_merged() -> pd.DataFrame:
    ...
    merged = df_all.merge(
        commit_df[["project", "commit_id", "commit_date"]], on="commit_id", how="left"
    )
    return merged.dropna(subset=["commit_date"])
...
def complete(project: str, commit: str, partial_msg: str) -> str:
    user_prompt = build_prompt(project, commit, partial_msg)
...
def evaluate(df: pd.DataFrame, name: str) -> dict[str, float | int | str]:
    ...
    for proj, cid, true_msg in tqdm(iterator, total=len(df), desc=f"{name:>18}"):
        ...
        try:
            ...
            print(f"DeepSeek / OpenAI API error – treating as empty: {e}")
            pred = ""
